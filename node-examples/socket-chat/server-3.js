...
io.on('connection', (socket) => {
    ...
    socket.emit('load old messages', chatHistory);

    socket.on('chat message', (msg) => {
        const fullMessage = `${socket.username}: ${msg}`;
        chatHistory.push(fullMessage);  // Save message to history
        if (chatHistory.length > 100) {  // Limit history to last 100 messages
            chatHistory.shift();
        }
